...
uint8_t MFRC522_Check(SPI_TypeDef* SPIx, uint8_t * id) {
	uint8_t status;
	status = MFRC522_Request(SPIx, PICC_REQIDL, id);	// Find cards, return card type
	if (status == MI_OK) status = MFRC522_Anticoll(SPIx, id);	// Card detected. Anti-collision, return card serial number 4 bytes
...
void MFRC522_Write(SPI_TypeDef* SPIx, uint8_t addr, uint8_t val)
{
	...
	addr = (addr << 1) & 0x7E;
	SPI_I2S_SendData(SPIx, (uint16_t) addr);
...
uint8_t MFRC522_Read(SPI_TypeDef* SPIx, uint8_t addr)
{
	...
	val = (SPI_I2S_ReceiveData(SPIx) << 8);
	while(SPI_I2S_GetFlagStatus(SPI1, SPI_I2S_FLAG_BSY) == SET);
...
{
	uint8_t tmp;
	tmp = MFRC522_Read(SPIx, reg);
	MFRC522_Write(SPIx, reg, tmp | mask);  // set bit mask
}
...
void MFRC522_ClearBitMask(SPI_TypeDef* SPIx, uint8_t reg, uint8_t mask)
{
	uint8_t tmp;
	tmp = MFRC522_Read(SPIx, reg);
	MFRC522_Write(SPIx, reg, tmp & (~mask));
}
...
void MFRC522_AntennaOn(SPI_TypeDef* SPIx)
{
	...
	temp = MFRC522_Read(SPIx, MFRC522_TxControlReg);
	if (!(temp & 0x03))
	{
		MFRC522_SetBitMask(SPIx, MFRC522_TxControlReg, 0x03);
	}
}

void MFRC522_AntennaOff(SPI_TypeDef* SPIx)
{
	MFRC522_ClearBitMask(SPIx, MFRC522_TxControlReg, 0x03);
}

void MFRC522_Reset(SPI_TypeDef* SPIx)
{
	MFRC522_Write(SPIx, MFRC522_CommandReg, PCD_RESETPHASE);
}
...
uint8_t MFRC522_Request(SPI_TypeDef* SPIx, uint8_t reqMode, uint8_t *TagType)
{
	...
	MFRC522_Write(SPIx, MFRC522_BitFramingReg, 0x07);											// TxLastBists = BitFramingReg[2..0]
	TagType[0] = reqMode;
...
uint8_t MFRC522_ToCard(SPI_TypeDef* SPIx, uint8_t command, uint8_t *sendData, uint8_t sendLen, uint8_t *backData, uint8_t *backLen)
{
	uint8_t status = MI_ERR;
		...
		MFRC522_Write(SPIx, MFRC522_ComlEnReg, irqEn | 0x80);
		MFRC522_ClearBitMask(SPIx, MFRC522_ComIrqReg, 0x80);
		MFRC522_SetBitMask(SPIx, MFRC522_FIFOLevelReg, 0x80);
		MFRC522_Write(SPIx, MFRC522_CommandReg, PCD_IDLE);

		// Writing data to the FIFO
		for (i = 0; i < sendLen; i++) MFRC522_Write(SPIx, MFRC522_FIFODataReg, sendData[i]);

		// Execute the command
		MFRC522_Write(SPIx, MFRC522_CommandReg, command);
		if (command == PCD_TRANSCEIVE) MFRC522_SetBitMask(SPIx, MFRC522_BitFramingReg, 0x80);					// StartSend=1,transmission of data starts
		...
		do {
			...
			n = MFRC522_Read(SPIx, MFRC522_ComIrqReg);
			i--;
		} while ((i!=0) && !(n&0x01) && !(n&waitIRq));

		MFRC522_ClearBitMask(SPIx, MFRC522_BitFramingReg, 0x80);												// StartSend=0

		if (i != 0)  {
			if (!(MFRC522_Read(SPIx, MFRC522_ErrorReg) & 0x1B)) {
				status = MI_OK;
				if (n & irqEn & 0x01) status = MI_NOTAGERR;
				if (command == PCD_TRANSCEIVE) {
					n = MFRC522_Read(SPIx, MFRC522_FIFOLevelReg);
					lastBits = MFRC522_Read(SPIx, MFRC522_ControlReg) & 0x07;
					if (lastBits) *backLen = (n - 1) * 8 + lastBits; else *backLen = n * 8;
					if (n == 0) n = 1;
					if (n > MFRC522_MAX_LEN) n = MFRC522_MAX_LEN;
					for (i = 0; i < n; i++) backData[i] = MFRC522_Read(SPIx, MFRC522_FIFODataReg);		// Reading the received data in FIFO
				}
			} else status = MI_ERR;
		}
		return status;
...
uint8_t MFRC522_Anticoll(SPI_TypeDef* SPIx, uint8_t *serNum)
{
	...
	MFRC522_Write(SPIx, MFRC522_BitFramingReg, 0x00);												// TxLastBists = BitFramingReg[2..0]
...
void MFRC522_CalculateCRC(SPI_TypeDef* SPIx, uint8_t *pIndata, uint8_t len, uint8_t *pOutData)
{
	...
	MFRC522_ClearBitMask(SPIx, MFRC522_DivIrqReg, 0x04);													// CRCIrq = 0
	MFRC522_SetBitMask(SPIx, MFRC522_FIFOLevelReg, 0x80);													// Clear the FIFO pointer
	// Write_MFRC522(CommandReg, PCD_IDLE);

	// Writing data to the FIFO
	for (i = 0; i < len; i++) MFRC522_Write(SPIx, MFRC522_FIFODataReg, *(pIndata+i));
	MFRC522_Write(SPIx, MFRC522_CommandReg, PCD_CALCCRC);

	// Wait CRC calculation is complete
	i = 0xFF;
	do {
		n = MFRC522_Read(SPIx, MFRC522_DivIrqReg);
		i--;
	} while ((i!=0) && !(n&0x04));																		// CRCIrq = 1

	// Read CRC calculation result
	pOutData[0] = MFRC522_Read(SPIx, MFRC522_CRCResultRegL);
	pOutData[1] = MFRC522_Read(SPIx, MFRC522_CRCResultRegH);
}
...
uint8_t MFRC522_Auth(SPI_TypeDef* SPIx, uint8_t authMode, uint8_t BlockAddr, uint8_t *Sectorkey, uint8_t *serNum)
{
	...
	if ((status != MI_OK) || (!(MFRC522_Read(SPIx, MFRC522_Status2Reg) & 0x08))) status = MI_ERR;
...
void MFRC522_Halt(SPI_TypeDef* SPIx)
{
	...
	buff[1] = 0;
	MFRC522_CalculateCRC(SPIx, buff, 2, &buff[2]);
	MFRC522_ToCard(SPIx, PCD_TRANSCEIVE, buff, 4, buff, &unLen);
}
